...
For an end-to-end example on a real model, check out the end-to-end torch.compile tutorial.
...
**Contents:** Basic Usage, Demonstrating Speedups, Benefits over TorchScript, Graph Breaks, Troubleshooting, Conclusion

**Required pip dependencies for this tutorial:** `torch >= 2.0`, `numpy`, `scipy`
...
Now let's demonstrate how `torch.compile` speeds up a simple PyTorch example. For a demonstration on a more complex model, see the end-to-end torch.compile tutorial.
...
```
compile: 0.550845703125
eager: 0.030170976638793947
```

Notice that `torch.compile` appears to take a lot longer to complete compared to eager. This is because `torch.compile` takes extra time to compile the model on the first few executions. `torch.compile` re-uses compiled code whenever possible, so if we run our optimized model several more times, we should see a significant improvement compared to eager.
...
```
(eval) eager median: 0.0008662079870700836, compile median: 0.00035740800201892857, speedup: 2.4235830820156306x
```
...
torch._dynamo.exc.Unsupported: Data-dependent branching
  Explanation: Detected data-dependent branching (e.g. `if my_tensor.sum() > 0:`). Dynamo does not support tracing dynamic control flow.
      ...
      The branch condition involves a tensor computed as follows:
        # File "torch_compile_tutorial.py", line 313, in bar, code: if b.sum() < 0:
        lt = lt(sum_1, 0)
...
Check out the section on graph breaks in the torch.compile programming model for tips on how to work around graph breaks.
