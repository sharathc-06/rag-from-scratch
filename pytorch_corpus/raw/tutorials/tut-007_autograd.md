...
(diagram: input x -> matmul with w, plus b -> z -> binary_cross_entropy_with_logits with y -> loss)
...
To optimize weights of parameters in the neural network, we need to compute the derivatives of our loss function with respect to parameters, namely, we need the partial derivative of loss with respect to w and with respect to b under some fixed values of `x` and `y`. To compute those derivatives, we call `loss.backward()`, and then retrieve the values from `w.grad` and `b.grad`:
...
For a vector function y = f(x), where x = (x_1, ..., x_n) and y = (y_1, ..., y_m), a gradient of y with respect to x is given by the **Jacobian matrix** J, whose (i,j) entry is the partial derivative of y_i with respect to x_j.
