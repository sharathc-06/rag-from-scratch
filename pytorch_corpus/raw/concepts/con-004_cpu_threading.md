...
Source: https://docs.pytorch.org/docs/stable/notes/cpu_threading_torchscript_inference.html
(Fetched via canonical GitHub source: github.com/pytorch/pytorch/blob/master/docs/source/notes/cpu_threading_torchscript_inference.rst)
...
|---|---|---|
| Inter-op parallelism | `at::set_num_interop_threads`, `at::get_num_interop_threads` (C++), `torch.set_num_interop_threads`, `torch.get_num_interop_threads` (Python, `torch` module) | Default number of threads: number of CPU cores. |
| Intra-op parallelism | `at::set_num_threads`, `at::get_num_threads` (C++), `torch.set_num_threads`, `torch.get_num_threads` (Python, `torch` module). Environment variables: `OMP_NUM_THREADS`, `MKL_NUM_THREADS` | |
...
Note: Pre-built PyTorch releases are compiled with OpenMP support.
